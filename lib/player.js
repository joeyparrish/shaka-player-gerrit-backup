...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
    ...
    this.eventManager_.listenOnce(mediaElement, 'loadeddata', () => {
      ...
      this.stats_.reportToAnalytics(this);
    });
  }
   ...
   * @private
   */
  googleAnalyticsCall_(method, obj) {
    const funcName = window['GoogleAnalyticsObject'];
    if (this.config_.enableAnalytics && funcName) {
      window[funcName](method, obj);
    }
  }

  /**
   * Sends an "page view" to the analytics.
   * @param {string} pageName
   * @export
   */
  sendAnalyticsPageView(pageName) {
    this.googleAnalyticsCall_('send', {
      hitType: 'pageview',
      page: pageName,
    });
  }
   ...
   * This is data that will show up in the "events" tab under "realtime" in the
   * analytics page.
  ...
  /**
   * Sets a "metric" on the analytics.
   * @param {string} field
   * @param {string|number} value
   * @export
   */
  setAnalyticsMetric(field, value) {
    const fieldsObject = {};
    fieldsObject[field] = value;
    fieldsObject['hitType'] = 'pageview';
    this.googleAnalyticsCall_('send', fieldsObject);
  }
  ...
  onKeyStatus_(keyStatusMap) {
    ...
    if (activeVariant && !activeVariant.allowedByKeySystem) {
      ...
      this.chooseStreamsAndSwitch_(currentPeriod);
    }
 ...
 * @export
 */
shaka.Player.version = 'v2.5.5-master-uncompiled';
