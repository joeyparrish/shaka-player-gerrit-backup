...
shaka.media.StreamingEngine = function(manifest, playerInterface) {
  ...
  /**
   * List of identifiers for MP4 emsg boxes found and triggered as player
   * events.
   *
...
shaka.media.StreamingEngine.prototype.parseEMSG_ = function(
  ...
  // Only parses emsg version 0.
  // TODO: https://github.com/google/shaka-player/issues/1539 Support for
  // DASHEventMessageBox Version 1.
  if (box.version != 0) {
    return;
  }
  ...
  // See DASH sec. 5.10.3.3.4
  // Processing of any one event message box with the same id is sufficient.
  if (this.mp4EmsgIds_.indexOf(id) != -1) {
    return;
  }
  this.mp4EmsgIds_.push(id);
