...
shaka.hls.HlsParser.widevineDrmParser_ = function(drmTag) {
  ...
  // CENC (CTR mode) is supported in Widevine everywhere.
  var supportedMethods = ['SAMPLE-AES-CENC'];

  // CBCS (CBC mode) is only supported on some platforms.
  if (window.cast && cast.__platform__ &&
      cast.__platform__.queryPlatformValue &&
      cast.__platform__.queryPlatformValue('enable-hls-sample-aes')) {
    supportedMethods.push('SAMPLE-AES');
  }

  if (supportedMethods.indexOf(method) == -1) {
    shaka.log.error(
        'Widevine in HLS is only supported with', supportedMethods, ', not',
        method);
    return null;
  }
