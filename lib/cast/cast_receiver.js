...
shaka.cast.CastReceiver = class extends shaka.util.FakeEventTarget {
   ...
   * @param {boolean=} fakeCastMode If true, use BroadcastChannel to communicate
   *   with a sender app in another tab or window in the same browser instance.
   *   This makes it easier to debug a Cast receiver app in a normal browser.
   *   This feature is only available for debugging.  The parameter is ignored
   *   in compiled mode.
   */
  constructor(video, player, appDataCallback, contentIdCallback, fakeCastMode) {
    super();
    ...
    // the receiver app.  This can be triggered by opening the demo app and
    // Cast receiver app in two different tabs or windows of the same browser
    // instance, then appending ";fakeCast" to the end of both URLs and
    // reloading the tabs.
    if (goog.DEBUG && fakeCastMode) {
      /** @type {!Map.<string, !BroadcastChannel> } */
      const channels = new Map();

      let fakeManagerInstance = null;
      ...
      cast.receiver.CastChannel = class {
        ...
        /** @param {*} data */
        send(data) {
          this.channel_.postMessage(data);
        }
      };
      ...
      cast.receiver.CastMessageBus = class {
        ...
        constructor(namespace) {
          let channel = channels.get(namespace);

          if (!channel) {
            channel = new BroadcastChannel('fakeCast:' + namespace);
            channel.onmessage = (event) => {
              if (this.onMessage) {
                this.onMessage({data: event.data, senderId: 'fakeSender'});
              }
            };
        ...
        getCastChannel(senderId) {
          return new cast.receiver.CastChannel(this.channel_);
        }
      ...
      cast.receiver.CastReceiverManager = class {
        ...
        static getInstance() {
          if (!fakeManagerInstance) {
            fakeManagerInstance = new cast.receiver.CastReceiverManager();
          }
        ...
        getSystemVolume() {
          return {level: video.volume, muted: video.muted};
        }
        ...
        getCastMessageBus(namespace) {
          return new cast.receiver.CastMessageBus(namespace);
        }
      };
    }
   ...
   * @private
   */
  sendMediaStatus_(requestId, media) {
    const mediaStatus = {
