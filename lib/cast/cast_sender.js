...
shaka.cast.CastSender = class {
  ...
  onSessionCreated_(session) {
    goog.asserts.assert(this.onMessageReceivedBound_, 'Should not be null!');
    goog.asserts.assert(
        this.onConnectionStatusChangedBound_, 'Should not be null!');
  ...
  onConnectionStatusChanged_() {
    const status = shaka.cast.CastSender.session_ ?
        shaka.cast.CastSender.session_.status :
        chrome.cast.SessionStatus.DISCONNECTED;
    const connected = status == chrome.cast.SessionStatus.CONNECTED;

    shaka.log.debug('CastSender: connection status', status, connected);
  ...
  /**
   * In debug mode, we can force a "fake" Cast mode to make it easier to test
   * the receiver app.  This can be triggered by opening the demo app and Cast
   * receiver app in two different tabs or windows of the same browser instance,
   * then appending ";fakeCast" to the end of both URLs and reloading the tabs.
   *
   * This method will be called by shaka.cast.CastProxy, which is called by the
   * demo app, and will not be available in compiled mode.
   */
  enableFakeCastMode() {
    if (goog.DEBUG) {
      // If we're casting, stop.
      this.forceDisconnect();

      // If there's a callback for the real Cast SDK, remove it.
       ...
       */
      const getChannel = (namespace) => {
        const internalNamespace = 'fakeCast:' + namespace;
        let channel = channels.get(internalNamespace);
        if (!channel) {
          channel = new BroadcastChannel(internalNamespace);
          channels.set(internalNamespace, channel);
        }
        return channel;
      };
      ...
      chrome.cast.Session = class {
        ...
        addMessageListener(namespace, listener) {
          const channel = getChannel(namespace);
          channel.onmessage = (event) => {
            listener(namespace, event.data);
          };
        }

        removeUpdateListener() {}

        removeMessageListener(namespace) {
          const channel = getChannel(namespace);
      ...
      chrome.cast.requestSession = (onSession) => {
        if (this.isCasting_) {
          // In the real world, this would trigger the cast dialog and allow the
          // user to disconnect.  With this fake, we will just disconnect.
          this.sendMessage_({
            'type': 'asyncCall',
            'targetName': 'player',
            'methodName': 'unload',
      ...
      // Reinitialize with the fakes installed.
      this.init();
    }  // if (goog.DEBUG)
  }  // enableFakeCastMode()
};  // CastSender = class
