...
goog.require('shaka.offline.AssignStreamInfo');
...
shaka.offline.Storage = class {
  ...
  async store_(uri, appMetadata, getParser, config, downloader) {
    ...
    try {
      ...
      // TODO: I think with BG fetch, "manifestDB" could have changed inside
      // |this.downloadSegments_|... which would mean that manifestDB will
      // no longer be accurate!
      // So in that case, re-load the manifest...
  ...
  async downloadSegments_(
    ...
    const download = async (toDownload, updateDRM) => {
      ...
      for (const download of toDownload) {
        ...
        const onDownloaded = (d) => {
          toAssign.push(new shaka.offline.AssignStreamInfo(
              download.ref, {data: d}));
      ...
      manifestDB = (await shaka.offline.Storage.assignStreamsToManifest(
          manifestId, toAssign, throwIfAbortedFn)) || manifestDB;
      ...
      if (updateDRM) {
        ...
        // process of downloading and storing, and assignStreamsToManifest does
    ...
    const usingBgFetch = true; // TODO: Get.
    ...
    if (!usingBgFetch) {
      ...
      await shaka.offline.Storage.backgroundFetch_(
          toDownload, config, manifestId);
    }
  }
   ...
   * @private
   */
  static async backgroundFetch_(toDownload, config, manifestId) {
    // TODO: convert any range requests that don't fit the format of a 'simple'
    // range request... to quote Chromium:
    //    A 'simple' range value is of the following form: 'bytes=\d+-(\d+)?'.
    // So we will want to convert any range requests that aren't in the unit
    // of bytes, has multiple ranges (is this common?), or uses a 'negative'
    // (e.g. from the end) range.

    // TODO: I forget if I want to have anything special for the loading process
    // or if we just wanted to require the user to load our service worker code
    // into their service worker...
    const reg = await navigator.serviceWorker.ready;
    const id = 'Shaka-' + manifestId;
    const requests = toDownload.map((info) => {
      const request = info.makeSegmentRequest(config);
      ...
      // Convert the Shaka request into a Fetch request.
      const init = {
        method: request.method,
        headers,
        body: request.body || undefined, // TODO: do I need "|| undefined" here?
        credentials: request.allowCrossSiteCredentials ? 'include' : undefined,
        // TODO: set a cache mode?
        // TODO: redirect mode?
        // TODO: referrer mode?
        // TODO: integrity?
      };
      return new Request(request.uris[0], init);
    });
    const options = {
      title: 'Shaka Player Download Funtimes!!',
      // TODO: any "icons" value?
      // TODO: calculate a "downloadTotal" value
    };
    // TODO: make an extern for this:
    ...
    bgFetch.addEventListener('progress', (event) => {
      // TODO: handle the progress events
      console.log('progress', event, bgFetch);
    });
    // TODO: I'm getting "fetch-error"...
    //    "fetch-error" - One of the fetches failed for some other reason, e.g.
    //    CORS, MIX, an invalid partial response, or a general network failure
    //    for a fetch that cannot be retried.
    // How annoyingly nonspecific.
    // Honestly, a CORS error sounds most likely...
    // TODO: So are the CORS headers on our demo assets set incorrectly?
    // Ugh, I don't know how to change these things...
    // TODO: Also I think I need to add a handler for the cancel or something?
    // Because these BG fetches refuse to go away on their own, even if I select
    // "cancel"...
  }
   ...
   *
   * @param {number} manifestId
   * @param {!Array.<!shaka.offline.AssignStreamInfo>} toAssign
   ...
   * @export
   */
  static async assignStreamsToManifest(manifestId, toAssign, throwIfAbortedFn) {
    /** @type {!Map.<
     * shaka.media.SegmentReference|shaka.media.InitSegmentReference, string
     * >} */
    const idsForRefs = new Map();
    for (const {ref, data} of toAssign) {
      if (ref) {
        const idForRef = shaka.offline.DownloadInfo.idForSegmentRef(ref);
        idsForRefs.set(ref, idForRef);
      }
    }
    const changes = shaka.offline.DownloadInfo.translateRefIds(
        new Set(idsForRefs.values()));
    for (const ref of idsForRefs.keys()) {
      if (ref) {
        const oldRefId = idsForRefs.get(ref);
        idsForRefs.set(ref, changes.get(oldRefId));
      }
    }
    ...
    try {
      ...
      throwIfAbortedFn();
      // TODO: after this refactor, do I really need this mutex anymore?

      // Store the data.
      const refIdsToData = new Map();
      const storePromises = [];
      for (const {ref, data} of toAssign) {
        storePromises.push((async () => {
          const addSegmentResult = await activeHandle.cell.addSegments([data]);
          const dataKey = addSegmentResult[0];
          dataKeys.push(dataKey);
          throwIfAbortedFn();

          const idForRef = idsForRefs.get(ref);
          refIdsToData.set(idForRef, dataKey);
        })());
      }
      ...
      for (const stream of manifestDB.streams) {
        for (const segment of stream.segments) {
          if (refIdsToData.has(segment.pendingSegmentRefId)) {
            segment.dataKey = refIdsToData.get(segment.pendingSegmentRefId);
          ...
          if (refIdsToData.has(segment.pendingInitSegmentRefId)) {
            segment.initSegmentKey =
                refIdsToData.get(segment.pendingInitSegmentRefId);
      ...
      if (activeHandle && !manifestUpdated && dataKeys.length > 0) {
  ...
  makeManifestDB_(drmEngine, manifest, uri, metadata, config, downloader) {
    ...
    // Translate the pending ref ids, to ensure that the service worker is able
    // to later re-construct them.
    const refIds = new Set();
    for (const stream of streams) {
      for (const segment of stream.segments) {
        if (segment.pendingSegmentRefId) {
          refIds.add(segment.pendingSegmentRefId);
        }
        if (segment.pendingInitSegmentRefId) {
          refIds.add(segment.pendingInitSegmentRefId);
        }
      }
    }
    const changes = shaka.offline.DownloadInfo.translateRefIds(refIds);
    for (const stream of streams) {
      for (const segment of stream.segments) {
        if (segment.pendingSegmentRefId) {
          segment.pendingSegmentRefId =
              changes.get(segment.pendingSegmentRefId);
        }
        if (segment.pendingInitSegmentRefId) {
          segment.pendingInitSegmentRefId =
              changes.get(segment.pendingInitSegmentRefId);
        }
      }
    }
