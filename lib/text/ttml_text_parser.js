...
shaka.text.TtmlTextParser = class {
  ...
  parseMedia(data, time) {
    ...
    // If there are multiple XML tags (i.e. there were multiple ttml files
    // smooshed together), parse each separately.
    const split = str.split('<?xml ').filter((str) => str)
        .map((str) => '<?xml ' + str);
    if (split.length > 1) {
      let ret = [];
      for (let i = 0; i < split.length; i++) {
        ...
        const buffer = BufferUtils.toUint8(StringUtils.toUTF8(split[i]));
        ret = ret.concat(this.parseMedia(buffer, time));
      }
      return ret;
    }
