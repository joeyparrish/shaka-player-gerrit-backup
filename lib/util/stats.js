...
shaka.util.Stats = class {
  ...
  reportToAnalytics(player) {
    // TODO: Would it make sense to have config fields for the (optional) metric
    // numbers for each metric?
    // So if they say that buffering time is metric #2, then we fill it out like
    // that... and if they don't provide a number, we skip that metric.
    if (!isNaN(this.loadLatencySeconds_)) {
      player.setAnalyticsMetric('metric1', this.loadLatencySeconds_);
    }
    const bufferingTime = this.stateHistory_.getTimeSpentIn('buffering');
    player.setAnalyticsMetric('metric2', bufferingTime);
  }
